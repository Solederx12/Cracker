...
static void aimbotTab()
{
    ...
    if (gGlobalVars->features.automatic == 2) {
        ImGui::Text("power control");
        addPopupMenu(
            "power",
            []() {
                ImGui::PushItemWidth(150.f);
                ImGui::Checkbox("humanized power", &gGlobalVars->features.humanizedPower);
                if (gGlobalVars->features.humanizedPower) {
                    ImGui::SliderFloat("step size", &gGlobalVars->features.powerStepSize, 0.05f, 0.2, "%.5f", ImGuiSliderFlags_AlwaysClamp);
                    addTooltip("keep it low for legit");
                }
                else
                {
                    ImGui::SliderFloat("play power", &gGlobalVars->features.autoPlayMaxPower, 10.f, 100.f, "%.2f%%", ImGuiSliderFlags_AlwaysClamp);
                }
        ...
        ImGui::Text("angle rotation control");
        addPopupMenu(
            "angle rotation",
            []() {
                ImGui::PushItemWidth(150.f);
                ImGui::Checkbox("humanized angle rotation", &gGlobalVars->features.humanizedAngleRotation);
                if (gGlobalVars->features.humanizedAngleRotation) {
                    ImGui::SliderFloat("step size", &gGlobalVars->features.angleRotationStepSize, 0.005f, 0.5f, "%.5f", ImGuiSliderFlags_AlwaysClamp);
                    addTooltip("keep it low for legit");
                    ImGui::SliderInt("delay between steps", &gGlobalVars->features.delayBetweenAngleSteps, 1, 50, "%d ms", ImGuiSliderFlags_AlwaysClamp);
                }
    ...
    {
    case 0:
        ImGui::SliderFloat("delay", &gGlobalVars->features.autoPlayDelay, 0.f, 15.f, "%.3f", ImGuiSliderFlags_AlwaysClamp);
        break;
    default:
        ImGui::SliderFloat2("delay bounds", &gGlobalVars->features.autoPlayDelayBounds[0], 0.f, 5.f, "%f", ImGuiSliderFlags_AlwaysClamp);
        break;
    }
...
{
    ImGui::Checkbox("draw prediction", &gGlobalVars->features.esp.predicionPath);
    if (gGlobalVars->features.esp.predicionPath) {
        ImGui::PushID("predicionPath");
        ImGui::SliderInt("prediction draw power", &gGlobalVars->features.esp.displayPercantage, 10, 100, "%d%%");
        ImGui::SliderFloat("line thickness", &gGlobalVars->features.esp.ballLineThickness, 1.f, 10.f);
        ImGui::SliderFloat("ball circle radius", &gGlobalVars->features.esp.ballCircleRadius, 1.f, 40.f);
        ImGui::SliderFloat("ball transparency", &gGlobalVars->features.esp.ballTransparency, 0.f, 100.f, "%.f%%");
        ImGui::Checkbox("ball circle filled", &gGlobalVars->features.esp.ballCircleFilled);
        ImGui::PopID();
    }

    ImGui::Checkbox("draw shot state", &gGlobalVars->features.esp.shotState);
    if (gGlobalVars->features.esp.shotState) {
        ImGui::PushID("shot state");
        ImGui::SliderFloat("circle tickness", &gGlobalVars->features.esp.shotStateCircleThickness, 1.f, 10.f);
        ImGui::SliderFloat("circle radius", &gGlobalVars->features.esp.shotStateCircleRadius, 1.f, 50.f);
        ImGui::SliderFloat("circle transparency", &gGlobalVars->features.esp.shotStateTransparency, 0.f, 100.f, "%.f%%");
        ImGui::Checkbox("circle filled", &gGlobalVars->features.esp.shotStateCircleFilled);
        ImGui::PopID();
    }

    ImGui::Combo("prediction state", &gGlobalVars->features.esp.state, "player\0opponent\0both\0");

    drawSeparator();

    if (ImGui::Checkbox("wide guideline", &gGlobalVars->features.esp.wideGuideLine))
        UserSettings::setWideGuideLine(gGlobalVars, gGlobalVars->features.esp.wideGuideLine);

    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("requires rejoin.\n");
}
...
static void miscTab(bool& shouldBreak)
{
    ...
    drawSeparator();
    if (ImGui::Combo("colors style", &gGlobalVars->features.menu.colorsStyle, "gold\0classic\0dark\0light\0")) {
        updateMenuColorsStyle();
    }

    if (ImGui::Button("load", ImVec2(200.f, 0.f))) {
        Config::load();
        updateMenuColorsStyle();
    }

    if (ImGui::Button("save", ImVec2(200.f, 0.f)))
        Config::save();

    if (ImGui::Button("unload", ImVec2(200.f, 0.f)))
        shouldBreak = true;
}
